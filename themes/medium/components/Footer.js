  ...
  return (
    ...
    >
      ...
      <i className='fas fa-copyright' /> {`${copyrightDate}`} <span><i className='mx-1 animate-pulse fas fa-heart' /> <a href={BLOG.LINK} className='underline font-bold text-gray-500 dark:text-gray-300 '>{BLOG.AUTHOR}</a>.<br />
        ...
        <br />
        <h1>{title}</h1>
        <span className='text-xs font-serif'>Powered by <a href='https://github.com/UoToGK/NotionNext_blog' className='underline text-gray-500 dark:text-gray-300'>NotionNext {BLOG.VERSION}</a>.</span></span>
    </footer>
  )
}
