  ...
  return (
    ...
    >
      <span> © {`${copyrightDate}`} <span> <a href={BLOG.LINK} className='text-gray-500 dark:text-gray-300 '> <i className='mx-1 animate-pulse fas fa-heart' /> {BLOG.AUTHOR}</a>. <br /></span>
        ...
        <br />
        <span className='text-xs font-serif'> Powered by <a href='https://github.com/UoToGK/NotionNext_blog' className='underline text-gray-500 dark:text-gray-300'>NotionNext {BLOG.VERSION}</a></span><br /></span>
      <h1>{title}</h1>
    </footer>
  )
}
